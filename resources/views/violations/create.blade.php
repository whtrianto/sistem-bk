...
<div class="container-fluid fade-in-section">
    <div class="mb-4">
        <a href="{{ route('violations.index') }}" class="btn btn-link text-decoration-none p-0"><i class="bi bi-arrow-left"></i> Kembali ke Riwayat</a>
        ...
        <div class="col-lg-6">
            <div class="card border-0 glass-card p-4">
                <form action="{{ route('violations.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="violation_type_id" class="form-label fw-semibold">Jenis Pelanggaran <span class="text-danger">*</span></label>
                        <select name="violation_type_id" id="violation_type_id" class="form-select" required>
                            ...
                            @foreach($violationTypes as $type)
                                <option value="{{ $type->id }}" {{ old('violation_type_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }} - [{{ $type->category }} - Pengurangan {{ $type->points }} Poin]
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="date" class="form-label fw-semibold">Tanggal Kejadian <span class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" class="form-control" rows="3" placeholder="Masukkan kronologi singkat kejadian...">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div class="mb-3 form-check mt-3 bg-light p-3 rounded-3 border">
                        ...
                        <div class="form-text text-muted ps-0 mt-1">Sistem akan otomatis mengirim detail pelanggaran beserta sisa poin kredit siswa saat ini.</div>
                    </div>

                    <div class="text-end mt-4">
                        <button type="submit" class="btn btn-danger px-4"><i class="bi bi-exclamation-triangle me-2"></i> Simpan & Kurangi Poin</button>
                    </div>
                </form>
            </div>
        </div>
